...
def is_changepoint_in_valid_window(
...
def run(args):
  ...
  # for each test defined in the config file, if source
  # s big_query, config expects test_name, metrics_dataset,
  # metrics_table, project, metric_name as the keys for each test.
  # The above metadata is used to fetch the relevant data from BQ
  # for the test and run change point analysis.

  # change_point_sibling_distance, change_point_to_recent_run_window can be
  # defined in the config file for each test to avoid filing GitHub issues
  # for duplicate change points. Please take a look at the README for more
  # information on the parameters defined in the config file.
  ...
  for _, params in tests_config.items():
    ...
    if params['source'] == 'big_query':
      ...
      # (TODO): Implement fetching data from InfluxDB.
      params = None
    ...
    metric_values = data[load_test_metrics_utils.VALUE_LABEL]
    timestamps = data[load_test_metrics_utils.SUBMIT_TIMESTAMP_LABEL]
    ...
    if not is_changepoint_in_valid_window(change_point_to_recent_run_window,
                                          change_point_index):
    ...
    logging.info("Create alert for the test %s: %s" % (test_name, create_alert))
    ...
    if create_alert:
      ...
      metric_dict = GitHubIssueMetaData(
      ...
      time.sleep(30)
      bq_metrics_publisher.publish([metric_dict])
      logging.info(
...
if __name__ == '__main__':
  ...
  # run method runs change point analysis on test metric data, and if there is
  # a performance regression, an alert will filed with GitHub Issues.
  run(known_args)
